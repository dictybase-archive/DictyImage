...
my $css_folder   = catfile( $asset_folder, 'stylesheets', 'elastislide' );
my $js_folder    = catfile( $asset_folder, 'javascripts', 'elastislide' );
...
$mt->render_file_to_file(
    catfile( $Bin, updir, 'template', 'elastislide.html' ),
    $output,
    {   images     => $images,
