...
$mt->render_file_to_file(
    catfile( $Bin, updir, 'template', 'galleria.html' ),
    $output,
    {   images => [@files],
        css =>
            catfile( $asset_folder, 'stylesheets', 'galleria.classic.css' ),
        js => catfile(
            $asset_folder, 'javascripts', 'galleria-1.2.5.min.js'
        ),
        theme =>
            catfile( $asset_folder, 'javascripts', 'galleria.classic.min.js' ),
    }
);
